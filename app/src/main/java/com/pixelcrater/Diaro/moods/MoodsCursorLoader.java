...
public class MoodsCursorLoader extends CursorLoader implements OnStorageDataChangeListener {

    private final String mDialogSearchKeyword;

    public MoodsCursorLoader(Context context, String dialogSearchKeyword) {
        super(context);
    ...
    public Cursor loadInBackground() {
        ...
        if (!mDialogSearchKeyword.equals("")) {
            andSql = "AND f." + Tables.KEY_MOOD_TITLE + " LIKE ?";
            whereArgs = new String[1];
            whereArgs[0] = "%" + mDialogSearchKeyword + "%";
        }

        Cursor cursor = MyApp.getInstance().storageMgr.getSQLiteAdapter().getMoodsCursor( andSql, whereArgs, true);
